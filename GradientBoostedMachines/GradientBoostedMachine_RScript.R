################################################################################
######################## Gradient Boosted Machine Example ######################
################################################################################
...
# Remove any columns in the dataset that are free-text fields (e.g., raw dates) 
# or metadata fields that behave like identifiers (e.g., county)
# or generally non-meaningful for the prediction task
# This is to avoid leakage and to keep interpretation focused on ecological predictors

drop_extra <- c("county", "collection_date", "trap_problem_bit", "sex", "collection_type", "closest_longitude", "closest_latitude",
                "trap_problem_bit", "collection_longitude", "collection_latitude")
...
# Categorical predictors must be converted using one-hot encoding.
...
##### 6: Create testing & training matrices #####
...
##### 8. Run model using optimal hyperparameters identified above #####
